...
<br>
<script type="application/javascript" src="https://syndication.realsrv.com/splash.php?idzone=4148960"></script>
<script type="application/javascript" data-idzone="4148956" data-ad_first_trigger_clicks="1" data-ad_next_trigger_clicks="5" data-type="desktop" data-browser_settings="1" data-ad_trigger_method="3" src="https://a.realsrv.com/fp-interstitial.js"></script>
<script type="application/javascript">
    var ad_idzone = "4148954",
    ad_width = "300",
    ad_height = "250",
    v_pos = "bottom",
    h_pos = "center";
</script>
<script type="application/javascript" src="https://a.realsrv.com/js.php?t=17&idzone=4148954"></script>

<footer class="text-muted"> <div class="container">
</div> <p class="float-right"> <a href="https://redzone.fun">Home</a> </p> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://github.com/ParveenBhadooOfficial/Bhadoo-Drive-Index" target="_blank">TheFirstSpeedster</a>, based on Open Source Softwares.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a>, All Rights Reserved.</p> </div> </footer>
  `;
	$('body').html(html);
}
...
function append_files_to_list(path, files) {
	...
	var itemTotal = files.length;
	console.log("UI.Per_page is",UI.Per_page);
    var LstPage = Math.ceil(itemTotal / UI.Per_page);
    console.log("Lst page is",LstPage);

    var files =  files.splice((pgno-1)*(UI.Per_page), UI.Per_page);
	console.log(files);
	for (i in files) {
		var item = files[i];
  ...
  html += `<br><br> <nav aria-label="Page navigation example"><ul class="pagination">`;
  if (pgno > 1) {
  		html += `<li class="page-item"><a class="page-link" href="?p=${+pgno - 1}">Previous</a></li>`;
  }
  if (pgno > 2) {
  		html += `<li class="page-item"><a class="page-link" href="?p=1">1</a></li>`;
  	if (pgno > 3) {
  	    html += `<li class="page-item">
          <a class="page-link" href="?p=${+pgno - 2}" aria-label="Previous">
  ...
  if (pgno > 1) {
  	html += `<li class="page-item"><a class="page-link" href="?p=${+pgno - 1}">${+pgno - 1}</a></li>`;
  }

  html += `<li class="page-item"><a class="page-link" href="?p=${pgno}">${pgno}</a></li>`;
  ...
  if (pgno < LstPage-1) {
  	if (pgno < LstPage-2) {
  		html += `<li class="page-item">
     	  <a class="page-link" href="?p=${+pgno + 2}" aria-label="Next">
          <span aria-hidden="true">&raquo;</span>
          <span class="sr-only">Next</span>
      	</a>
    	</li>`;
  	}
  	html += `<li class="page-item"><a class="page-link" href="?p=${LstPage}">${LstPage}</a></li>`;
  }
  if (pgno < LstPage) {
  	html += `<li class="page-item"><a class="page-link" href="?p=${+pgno + 1}">Next</a></li>`;
  }
  html += `</ul></nav>`;
...
<p>Download Option is for VIP Users Only <a href="https://redzone.fun/contact.php?sub=VIP_Registration">Click here to become VIP</a></p>`;
	$('#content').html(content);
}
...
function file_video(path) {
  ...
  <iframe src="https://syndication.realsrv.com/ads-iframe-display.php?idzone=4148962&output=noscript" width="300" height="250" scrolling="no" marginwidth="0" marginheight="0" frameborder="0"></iframe>
  </div>
  `;
	$('#content').html(content);
}
...
$(function () {
	...
	if (check1.substr(check1.indexOf("p=")+2)) {
		pgno = check1.substr(check1.indexOf("p=")+2);
		console.log("pgno check1 is",pgno);
	} else {
		pgno = 0;
		console.log("nothing in pgno parameter");
	}
	var path = window.location.pathname;
