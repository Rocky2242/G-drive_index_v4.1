...
<footer class="text-muted"> <div class="container"> <p class="float-right"> <div id="pagination"></div> <a href="https://redzone.fun">Home</a> </p> ${UI.credit ? '<p>Redesigned with <svg width="1em" height="1em" viewBox="0 0 16 16" class="bi bi-heart-fill" fill="red" xmlns="http://www.w3.org/2000/svg"> <path fill-rule="evenodd" d="M8 1.314C12.438-3.248 23.534 4.735 8 15-7.534 4.736 3.562-3.248 8 1.314z" /> </svg> by <a href="https://github.com/ParveenBhadooOfficial/Bhadoo-Drive-Index" target="_blank">TheFirstSpeedster</a>, based on Open Source Softwares.</p>' : ''} <p>© ${UI.copyright_year} - <a href=" ${UI.company_link}" target="_blank"> ${UI.company_name}</a>, All Rights Reserved.</p> </div> </footer>
     <script src="//cdn.jsdelivr.net/gh/rocky2242/G-drive_index_v4.1/api/pagination.js"></script>
  <script src="//cdn.jsdelivr.net/gh/rocky2242/G-drive_index_v4.1/api/pagination.css"></script>
  `;
	$('body').html(html);
}
...
function append_files_to_list(path, files) {
    ...
    var files =  files.splice(pgno*16, 16);
	console.log(files);
	for (i in files) {
		var item = files[i];
