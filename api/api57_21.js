...
function list(path) {
  ...
  <div class="card">`;
  content += `<p><br><br> <nav aria-label="Page navigation example"><ul class="pagination">`;
  if (pgno > 1) {
  		content += `<li class="page-item"><a class="page-link" href="?p=${+pgno - 1}">Previous</a></li>`;
  }
  if (pgno > 2) {
  		content += `<li class="page-item"><a class="page-link" href="?p=1">1</a></li>`;
  	if (pgno > 3) {
  ...
  if (pgno > 1) {
  	content += `<li class="page-item"><a class="page-link" href="?p=${+pgno - 1}">${+pgno - 1}</a></li>`;
  }
  content += `<li class="page-item active"><a class="page-link" href="?p=${pgno}">${pgno}</a></li>`;
  if (pgno < LstPage) {
  	content += `<li class="page-item"><a class="page-link" href="?p=${+pgno + 1}">${+pgno + 1}</a></li>`;
  }
  if (pgno < LstPage-1) {
  	...
  	content += `<li class="page-item"><a class="page-link" href="?p=${LstPage}">${LstPage}</a></li>`;
  }
  if (pgno < LstPage) {
  	content += `<li class="page-item"><a class="page-link" href="?p=${+pgno + 1}">Next</a></li>`;
  }
  content += `</ul></nav></p>`;
  
  content += `<div id="readme_md" style="display:none; padding: 20px 20px;"></div>
  </div>
  `;
	...
	for (i in files) {
		...
		if (item['mimeType'] == 'application/vnd.google-apps.folder') {
          ...
          $("img").addClass("img-fluid")
			...
			if ("|mp4|webm|avi|bmp|jpg|jpeg|png|gif|m4a|mp3|flac|wav|ogg|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|pdf|".indexOf(`|${ext}|`) >= 0) {
				pn += "?a=view";
				c += " view";
